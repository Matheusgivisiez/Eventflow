...
export default function TicketTypesPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (ticketId: string) =>
      api(`/events/${eventId}/ticket-types/${ticketId}`, { method: "DELETE" }),
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Crie e gerencie os lotes de venda para este evento.
            </p>
          </div>
        </div>
        ...
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-14 text-center text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              Clique em "Novo lote" para adicionar o primeiro lote de ingressos.
            </p>
          </CardContent>
        </Card>
      )}
          ...
          editingId === ticket.id ? (
              ...
              <CardContent>
                <TicketForm
                  ...
                  defaultValues={{
                    ...
                    priceCents: ticket.priceCents / 100,
                    startsAt: ticket.startsAt.slice(0, 16),
                    endsAt: ticket.endsAt.slice(0, 16),
                    limitPerBuy: ticket.limitPerBuy,
                    isActive: ticket.isActive
                  }}
                  onSubmit={(data) =>
                    updateMutation.mutate({ ticketId: ticket.id, data })
                  }
            ...
            <TicketCard
              ...
              onDelete={() => {
                if (
                  confirm(
                    `Excluir o lote "${ticket.name}"? Esta ação não pode ser desfeita.`
                  )
                ) {
    ...
    <Card>
      <CardHeader className="flex-row items-start justify-between space-y-0 gap-4">
        ...
        <div className="flex gap-1 shrink-0">
          <Button variant="ghost" size="icon" onClick={onEdit} title="Editar lote">
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Metric
            label="Disponíveis"
            value={available.toLocaleString("pt-BR")}
        ...
        <div className="mt-4 space-y-1">
          ...
          <p className="text-xs text-muted-foreground pt-1">
            Vendas: {dateTime(ticket.startsAt)} → {dateTime(ticket.endsAt)}
          </p>
        </div>
      </CardContent>
    </Card>
...
function Metric({
  ...
  return (
    <div className="rounded-md bg-muted p-3">
      ...
      <p className={`mt-1 font-semibold${highlight ? " text-destructive" : ""}`}>
        {value}
      </p>
    </div>
  );
...
function TicketForm({
  ...
  defaultValues?: Partial<TicketForm>;
