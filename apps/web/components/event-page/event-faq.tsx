    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {faqs.map((faq, i) => (
          ...
          >
            <summary className="flex min-w-0 cursor-pointer items-center justify-between gap-3 font-medium text-foreground outline-none marker:content-none">
              <span className="flex min-w-0 flex-1 items-center gap-3">
                <HelpCircle className="h-5 w-5 shrink-0 text-primary/70" />
                <span className="min-w-0 break-words [overflow-wrap:anywhere]">{faq.question}</span>
              </span>
            ...
            <div className="mt-4 break-words pl-8 leading-relaxed text-muted-foreground [overflow-wrap:anywhere] animate-in fade-in slide-in-from-top-2">
              {faq.answer}
            </div>
          </details>
        ))}
